package com.KND.chequera.constant;

public class ViewConstant {
	
	public static final String LIST_CLIENTES_VIEW = "listClientes";
	public static final String ADD_CLIENTES_VIEW = "addCliente";
	
	public static final String LIST_BANCOS_VIEW = "listBancos";
	public static final String ADD_BANCOS_VIEW = "addBanco";
	
	public static final String LIST_TM_VIEW = "listTipoMovimientos";

	public static final String LIST_CH_CLIENTE_VIEW = "listChequerasClientes";
	public static final String ADD_CH_VIEW = "addChequera";
	
	public static final String LIST_MOVIMIENTOS_CHEQUERAS_VIEW = "listMovimientosChequeras";
	public static final String ADD_MOVIMIENTO_VIEW = "addMovimiento";
	
	public static final String ADD_EDOCUENTA_VIEW = "addEdoCuenta";
	
	public static final String INDEX_VIEW = "index";
	
	public static final String UTILERIAS_VIEW = "utilerias";
}
